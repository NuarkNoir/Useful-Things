var settings = {
	...
	updatesDir: null, autoEnter: false, fullScreen: false, shaders: false, opis: false, ram: 0, // Client
	...
	read: function(input) {
		...
		settings.opis = input.readBoolean();
	...
	write: function(output) {
		...
		output.writeBoolean(settings.opis);
	...
	setDefault: function() {
		...
		settings.opis = config.opisDefault;
	...
	initOverlay: function() {
		...
		// Shaders checkbox
		var shadersBox = settings.overlay.lookup("#shaders");
		shadersBox.setSelected(settings.shaders);
		shadersBox.selectedProperty()["addListener(javafx.beans.value.ChangeListener)"](
			function(o, ov, nv) settings.shaders = nv);

		// Opis checkbox
		var opisBox = settings.overlay.lookup("#opis");
		opisBox.setSelected(settings.opis);
		opisBox.selectedProperty()["addListener(javafx.beans.value.ChangeListener)"](
			function(o, ov, nv) settings.opis = nv);
...
var cliParams = {
	...
	updatesDir: null, autoEnter: null, fullScreen: null, shaders: null, opis: null, ram: -1, // Client
	...
	init: function(params) {
		...
		var shaders = named.get("shaders");
		if (shaders !== null) {
			cliParams.shaders = java.lang.Boolean.parseBoolean(shaders);
		}
		var opis = named.get("opis");
		if (opis !== null) {
			cliParams.opis = java.lang.Boolean.parseBoolean(opis);
		}
	...
	applySettings: function() {
		...
		if (cliParams.opis !== null) {
			settings.opis = cliParams.opis;
		}
		if (cliParams.ram >= 0) {
