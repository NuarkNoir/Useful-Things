...
function initDialog() {
	...
	rootPane.lookup("#bexit").setOnAction(function(event) javafx.application.Platform.exit());
	rootPane.lookup("#bhide").setOnAction(function(event) stage.setIconified(true));
	stage.initStyle(javafx.stage.StageStyle.TRANSPARENT);
    rootPane.setOnMousePressed(function(event) movePoint = new javafx.geometry.Point2D(event.getSceneX(), event.getSceneY()));
    rootPane.setOnMouseDragged(function(event) {
        if(movePoint === null) {
            return;
        }
...
function initAuthPane(pane) {
	...
	profilesBox.setVisibleRowCount(20)
	...
	// Lookup hyperlink text and actions
	var reg = pane.lookup("#reg");
	reg.setOnAction(function(event) app.getHostServices().showDocument(config.regURL.toURI()));
	
	// Lookup hyperlink text and actions
	var lk = pane.lookup("#lk");
	lk.setOnAction(function(event) app.getHostServices().showDocument(config.lkURL.toURI()));
	
	// Lookup hyperlink text and actions
	var status = pane.lookup("#status");
	status.setOnAction(function(event) app.getHostServices().showDocument(config.statusURL.toURI()));
	...
	overlay.swap(0, update.overlay, function(event) {
		...
		makeUpdateRequest(jvmDirName, jvmDir, null, function(jvmHDir) {
			...
			makeUpdateRequest(assetDirName, assetDir, assetMatcher, function(assetHDir) {
				...
				var clientMatcher = profile.object.getClientUpdateMatcher(settings.shaders, settings.opis);
...
function doLaunchClient(jvmDir, jvmHDir, assetDir, assetHDir, clientDir, clientHDir, profile, pp, accessToken) {
	...
	overlay.swap(0, processing.overlay, function(event)
		launchClient(jvmDir, jvmHDir, assetHDir, clientHDir, profile, new ClientLauncherParams(settings.lastSign,
			assetDir, clientDir, pp, accessToken, settings.autoEnter, settings.fullScreen, settings.shaders, settings.opis, settings.ram, 0, 0), doDebugClient)
	);
}
